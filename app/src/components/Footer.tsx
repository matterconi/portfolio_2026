...
export default async function Footer({ locale }: { locale: string }) {
  ...
  const quickLinks = SECTION_IDS.map((id) => ({
    id,
    label: tNav(id as any),
    href: `#${id}`,
  }));

  const socialObj = social as SocialLinks;
  const socialLinks = [
    { platform: 'github', label: 'GitHub', url: socialObj.github },
    { platform: 'linkedin', label: 'LinkedIn', url: socialObj.linkedin },
    { platform: 'email', label: 'Email', url: `mailto:${socialObj.email}` },
  ];
